...
{
    public class PriceListInsertHandler : IRequestHandler<PriceListInsertRequest, PriceList>
    {
        private readonly IPriceListRepository _repository;
        private readonly IRoomTypeRepository _roomTypeRepository;
        private readonly IRulesRepository _rulesRepository;

        public PriceListInsertHandler(IPriceListRepository repository, IRoomTypeRepository roomTypeRepository, IRulesRepository rulesRepository)
        {
        ...
        public async Task<PriceList> Handle(PriceListInsertRequest request, CancellationToken cancellationToken)
        {
            var isPriceValid = await IsRuleValidated(request.Price);
            if (isPriceValid)
                return await _repository.InsertPrice(request.Price);
            else
                return null;
        }
        ...
        private async Task<bool> IsRuleValidated(PriceList priceListObj)
        {
            ...
            if (rule is not null)
            {
                ...
                {
                    var roomTypeParent = (await _roomTypeRepository.Select(new RoomTypeQueryParameters { Id = roomType.Heirarchy.Value })).FirstOrDefault();

                    var parentRoomPrice = (await _repository.Select(new PriceListQueryParameters { IdRoomType = roomTypeParent.Id })).FirstOrDefault();
